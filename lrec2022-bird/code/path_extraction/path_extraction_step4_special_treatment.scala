 ...
 *  mvn scala:run -DmainClass=Path_Extraction_Step4_special_treatment "-DaddArgs=arg1|arg2"
 *  arg1: the part number for the output "filtered_paths_db_partxxx_subpartxxx.tsv" file
 *  arg2: the subpart number for the output "filtered_paths_db_partxxx_subpartxxx.tsv" file
 */
...
object Path_Extraction_Step4_special_treatment {
    def main(args:Array[String]) {
        ...
        */
        ...
        val subpart_no_int = args(1).toInt
        val subpart_no_str = "%03d".format(subpart_no_int)
        
        val paths_db_file = new File(filtered_paths_dbs_spec_treat_files_dir + "filtered_paths_db_part" + part_no_str_f + "_subpart_" + subpart_no_str + ".tsv")
        val paths_db_writer = new PrintWriter(paths_db_file, "UTF-8")
        
        val path_freq_idx = last_above_threshold_index_plus_one - num_of_last_huge_paths + (part_no_int_f - total_parts - 1)
        val path_f = paths_freq(path_freq_idx)._1
        
        if (subpart_no_int == 1)
        {
            val (slotX_f , slotY_f , sentences_info_f) = paths_db(path_f)
            
            val path_str_f = get_path_datastructure_string(path_f)
            val slotX_str_f = get_pathslot_datastructure_string(slotX_f)
            val slotY_str_f = get_pathslot_datastructure_string(slotY_f)
            val sentences_info_str_f = get_pathsent_datastructure_string(sentences_info_f, subpart_no_int, subparts_total)
            
            paths_db_writer.write(path_str_f + "\t" + slotX_str_f + "\t" + slotY_str_f + "\t" + sentences_info_str_f)
        }
        else
        {
            val (_ , _ , sentences_info_f) = paths_db(path_f)
            val sentences_info_str_f = get_pathsent_datastructure_string(sentences_info_f, subpart_no_int, subparts_total)
            paths_db_writer.write(sentences_info_str_f)
        }
    ...
    */
     ...
     */    
    def get_pathsent_datastructure_string(sentences_info: ArrayBuffer[(Int,Int,String,Int,Int,String,Int)] , subpart_no_int: Int, subparts_total: Int) : String =
    {
        val subpart_size: Int = (sentences_info.length) / subparts_total
        
        val loop_start = (subpart_no_int - 1) * subpart_size
        
        var loop_end = -1
        if (subpart_no_int == subparts_total)
        {
            loop_end = sentences_info.length
        }
        else
        {
            loop_end = subpart_no_int * subpart_size
        }
    
        var ret_val = ""
        
        for (i <- loop_start until loop_end)
        {
            val t = sentences_info(i)
