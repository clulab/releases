...
class Emboot:
    ...
    def __init__(self):
        ...
        ### Ontonotes data
        #########################################################################
        # parser.add_argument('--data', dest='data', default='./data/ontonotes_training_data_with_labels_pruned2.txt', help='data file')
        # parser.add_argument('--entity_vocab', dest='entity_vocab', default='./data/ontonotes_entity_vocabulary_pruned2.txt', help='entity vocabulary file')
        # parser.add_argument('--context_vocab', dest='context_vocab', default='./data/ontonotes_pattern_vocabulary_pruned2.txt', help='context vocabulary file')
        ...
        ### Output files
        #########################################################################
        parser.add_argument('--logfile', dest='logfile', default='pools_output_ablation.txt', help='entities promoted per epoch')
...
###################################################################################################
    ...
    def construct_pattern_embed_from_goldberg(self, patternIds_in_pool):
        pat_embedding_list = list()
        for pat in patternIds_in_pool:
            words_in_pat = [Gigaword.sanitiseWord(w) for w in self.context_vocab.get_word(pat).split(" ") if w != "@ENTITY"]
            embedIds_in_pat = [self.lookupGiga[w] if w in self.lookupGiga else self.lookupGiga["<unk>"] for w in words_in_pat]
            if len(embedIds_in_pat) > 1:
                pat_embedding = np.mean(Gigaword.norm(self.gigaW2vEmbed[embedIds_in_pat]), axis=0) # avg of the embeddings for each word in the pattern
            else:
                pat_embedding = Gigaword.norm(self.gigaW2vEmbed[embedIds_in_pat])

            pat_embedding_list.append(pat_embedding)

        return np.array([pe for pe in pat_embedding_list])

    def construct_entPool_embed_from_goldberg(self):

        mean_entPool_embeddings = list()
        for cat in self.categories:
            pool = self.model.pools_entities[cat]
            embed_entity_cat_pool = list()
            for eid in pool:
                entity = self.entity_vocab.get_word(eid)
                sanitised_entity = [Gigaword.sanitiseWord(tok) for tok in entity.split(" ")]
                giga_index = [self.lookupGiga[tok] if tok in self.lookupGiga else self.lookupGiga["<unk>"] for tok in sanitised_entity]
                embed = Gigaword.norm(self.gigaW2vEmbed[giga_index])
                if len(giga_index) > 1:
                    entity_embedding = np.mean(embed, axis=0)
                else:
                    entity_embedding = embed

                embed_entity_cat_pool.append(entity_embedding)

            ent_pool_mean = np.mean(np.array([embed for embed in embed_entity_cat_pool]), axis=0)
            mean_entPool_embeddings.append(ent_pool_mean)

        return np.array([ent_pool for ent_pool in mean_entPool_embeddings])

    def interpretable_prediction(self, patternIds):
        ...
        if len(patternIds_in_pool) > 0:
            ...
            pattern_embeddings = self.construct_pattern_embed_from_goldberg(patternIds_in_pool) # ablatioj test --> get the pattern embedding from the pretrained word-vectors instead of custom trained pat embeddings of Emboot
            if embed_emboot_global_feat_name not in self.feature_list or embed_emboot_feat_name not in self.feature_list:
                mean_entPool_embeddings = self.construct_entPool_embed_from_goldberg()
            else:
    ...
    def build_interpretable_model(self):
        ...
        for entityId, patternIds in self.entityToPatternsIdx.items():# for every entity in the dataset
            ...
            prediction, noisy_or_score = self.interpretable_prediction(patternIds)
            if prediction != -1:
