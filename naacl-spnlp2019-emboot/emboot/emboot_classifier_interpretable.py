...
class Emboot:
    ...
    def __init__(self):
        ...
        ### Output files
        #########################################################################
        parser.add_argument('--logfile', dest='logfile', default='pools_output_interpretable.txt', help='entities promoted per epoch')
        ...
        parser.add_argument('--pmi_type', dest='pmiType', default="pmi", help='type of pmi to be used for pattern promotion')
        ...
        print('Type of PMI used for pattern promotion : ', self.args.pmiType)
        ...
        self.pmiType = self.args.pmiType
        #########################################################################
        #########################################################################
...
###################################################################################################
    ...
    def compute_pattern_modpmi_logfreq(self, pattern, category):
        modpmi = self.compute_pattern_modpmi(pattern, category)
        patternCounts = self.context_vocab.get_count(pattern)
        return modpmi * math.log(patternCounts)

    def compute_pattern_modpmi(self, pattern, category):
        entities = self.patternToEntitiesIdx[pattern]
        entities_in_pool = set(self.model.pools_entities[category])
        entities_in_common = entities_in_pool.intersection(entities)
        ...
        ## NOT USING PATTERN COUNTS

        pmi = math.log(float(positiveEntityCounts) / countEntityMentions )

        return pmi

    def generate_entity_labels(self):
        self.entity_labels = dict()
        entity_label_pairs = { (eid, lbl) for eid, lbl in list(zip(self.mentions, self.labels)) }
        for eid, lbl in entity_label_pairs:
            if eid not in self.entity_labels:
                self.entity_labels[eid] = list()
            self.entity_labels[eid].append(lbl)

    def print_debug(self, entityOrPat, ent, pat, cat, epochId, score):
        if entityOrPat == 'pat':
            self.debug_log.write("[pat] Epoch: " + str(epochId) + "\tPromoting Pattern: " + self.context_vocab.get_word(pat) + "\tto\t" + cat + "\n")
            pat_entities = self.patternToEntitiesIdx[pat]
            for lbl, entities in self.model.pools_entities.items():
                matching_entities = list(set(entities).intersection(pat_entities))
                for e in matching_entities:
                    self.debug_log.write("\t[pat] " + self.entity_vocab.get_word(e) + "\t" + lbl + "\t" + str(self.entity_labels[e]) + "\n")
        elif entityOrPat == 'ent':
            self.debug_log.write("[ent] Epoch: " + str(epochId) + "\tPromoting Entity: "
                             + self.entity_vocab.get_word(ent) + "\tto\t" + cat + "\tScore(max, all): " + str(score) + "\n")
    ...
    def promote_patterns_by_pmi(self, promote_global, epochId):
        ...
        for cat in self.categories:
            ...
            for pat in preselected_patterns_non_overlapping:
                ...
                elif self.pmiType == "modpmi":
                    pmi = self.compute_pattern_modpmi(pat, cat)
                elif self.pmiType == "modpmilogfreq":
                    pmi = self.compute_pattern_modpmi_logfreq(pat, cat)
            ...
            if promote_global == -1.0:
                ...
                for pat, pmi in sorted_pmi_scores:
                    ...
                    if all(pat not in self.model.pools_contexts[c] for c in self.categories) and len(to_promote) < self.top_n:
                        ...
                        self.print_debug('pat', None, pat, cat, epochId, None)
...
###################################################################################################
    ...
    def compute_edit_distance_feature(self, entityId, category, features):
        ...
        #print("ENTITY, ENTITY_ID, CATEGORY, FEATURES:\t", entity, entityId, category, features)

        #print("POOLS ENTITIES?\t", self.model.pools_entities)

        # for e in self.model.pools_entities[category]:
        #     print("e:\t", e)
        #     print(editdistance.eval(self.entity_vocab.get_word(e), entity))
        #
        #     print(float(len(self.entity_vocab.get_word(e))))
        #
        #     print(float(editdistance.eval(self.entity_vocab.get_word(e), entity))/float(len(self.entity_vocab.get_word(e))))
        #
        #     if editdistance.eval(self.entity_vocab.get_word(e), entity)/float(len(self.entity_vocab.get_word(e))) < 0.2:
        #         print(1)
        #     else:
        #         print(0)
        ...
        posScore = max(
            [ 1
              if float(editdistance.eval(self.entity_vocab.get_word(e), entity))/float(len(self.entity_vocab.get_word(e))) < 0.2 ## [edit ditance] / |negEntStr|
              else
              0
              for e in self.model.pools_entities[category]
              if entityId != e ])
    ...
    def create_datum_features(self, ent):
        ...
        for cat in self.categories:
            if ed_feat_name in self.feature_list or ed_global_feat_name in self.feature_list:
                ...
                #print("CAT, ENT, ENT_FEATURES:\t", cat, ent, ent_features)
    ...
    def promote_entities_by_classifier(self, promote_global, epochId):
        ...
        if promote_global == -1.0:
            for idx,entid_pred in enumerate(candidate_entities_test_predictions_sorted.T):
                ...
                all_scores = classifier_results[sorting_index[idx]]
                scores = (log_prob_score, all_scores)
                ...
                if all(entid not in self.model.pools_entities[c] for c in self.categories) and len(to_promote[cat]) < self.top_n:
                    ...
                    self.print_debug('ent', entid, None, cat, epochId, scores)
...
###################################################################################################
    ...
    def interpretable_prediction(self, entityId, patternIds, epochId):

        patterns_pool = set([p for sublist in self.model.pools_contexts.values() for p in sublist])

        patternIds_in_pool = list(patterns_pool.intersection(patternIds))

        if len(patternIds_in_pool) > 0:
            pattern_embeddings = Gigaword.norm(self.model.get_context_embeddings()[patternIds_in_pool]) ## get the pattern embeddings of all patternIds occurring with this entityId
            mean_entPool_embeddings = np.array([self.model.get_mean_entPool_embedding(cat) for cat in self.categories])

            dot_products = np.dot(pattern_embeddings, mean_entPool_embeddings.T)
            if len(dot_products.shape) == 1: ## when only one overlapping pattern is found, then resultant dot prod shape id (embedding_sz) rathen than (1 x embedding sz), then softmax is failing. hence expanding dimensions
                dot_products = np.expand_dims(dot_products, axis=0)
            softmax_res = np.array([softmax(res) for res in dot_products]) ## prob(pattern , category)
            avg_softmax_res = np.average(softmax_res, axis=0)
            noisy_or_score = 1 - np.prod(1 - softmax_res, axis=0) # noisy or from the submission
            prediction = np.argmax(noisy_or_score)
            self.debug_log.write("[interpret] Epoch: " + str(epochId) + "\te=" + self.entity_vocab.get_word(entityId)
                                +"\tsz=" + str(len(softmax_res)) + "\tsm=" + str(avg_softmax_res) + "\tno-" + str(noisy_or_score) + "\tp-" + str(prediction) + "\n")

        else:
            noisy_or_score = np.array([0, 0, 0, 0])
            prediction = -1

        return prediction, noisy_or_score

    def build_interpretable_model(self, epochId):

        entityPredictions = dict()
        entityNoisyOrScore = dict()

        for entityId, patternIds in self.entityToPatternsIdx.items():# for every entity in the dataset
            ## Compute the noisy-or score of the entityId
            prediction, noisy_or_score = self.interpretable_prediction(entityId, patternIds, epochId)
            if prediction != -1:
                entityPredictions[entityId] = prediction
                entityNoisyOrScore[entityId] = noisy_or_score

        return entityPredictions, entityNoisyOrScore

    def print_interpretable_model(self, entity_predictions, entity_scores, epochId, interpretable_model_log):

        # interpretable_model_log.write("EPOCH: " + str(epochId) + "\n")
        for entId in entity_predictions.keys():
            ent = self.entity_vocab.get_word(entId)
            prediction = self.categories[entity_predictions[entId]]
            # print(scores)
            # scores = list(zip(self.categories, list(entity_scores[entId])))
            interpretable_model_log.write(str(epochId) + "\t" + ent + "\t" + prediction + "\t" + str(entity_scores[entId]) + "\n")

def softmax(x):
    """Compute softmax values for each sets of scores in x."""
    return np.exp(x) / np.sum(np.exp(x), axis=0)

def dump_embeddings(embeddings, vocabulary, file):
    with io.open(file, 'w', encoding='utf8') as f:
        for i in range(embeddings.shape[0]):
            f.write(vocabulary.get_word(i))
            f.write('\t')
...
if __name__ == '__main__':
    ...
    emboot.interpretable_model_log = open(emboot.args.logfile+"_interpretable_model.txt", 'w')

    emboot.debug_log = open(emboot.args.logfile+"_debug",'w')
    emboot.generate_entity_labels()
    ...
    for trial in range(n_trials):
        ...
        for rdx in range(n_rounds):
            ...
            print("Building the interpretable model")
            ## Build the interpretable model
            entity_predictions, entity_scores = emboot.build_interpretable_model((rdx+1))
            emboot.debug_log.flush()

            ## Score the interpretable model
            emboot.print_interpretable_model(entity_predictions, entity_scores, (rdx+1), emboot.interpretable_model_log)
    ...
    emboot.interpretable_model_log.close()
    emboot.debug_log.close()

    ### Dump the embeddings
    entity_embeddings = emboot.model.get_entity_embeddings()
    context_embeddings = emboot.model.get_context_embeddings()
    entity_embeddings_file = emboot.args.logfile+"_entemb.txt"
    context_embeddings_file = emboot.args.logfile+"_ctxemb.txt"
    dump_embeddings(entity_embeddings, emboot.entity_vocab, entity_embeddings_file)
    dump_embeddings(context_embeddings, emboot.context_vocab, context_embeddings_file)
