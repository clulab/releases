...
class Word2vec(object):
    ...
    def __init__(self, word_vocabulary, context_vocabulary, embedding_size, num_neg_samples, learning_rate, categories, usePatPool, initGigaEmbed, gigaW2vEmbed, lookupGiga, prior_entity_embedding=None, prior_pattern_embedding=None):
        ...
        if usePatPool:
            ...
            log_prob_eq = K.log(K.sigmoid(K.batch_dot(eq1_concat, eq2_concat, axes=1))+K.constant(0.0))
            log_prob_ne = K.log(K.sigmoid(-K.batch_dot(ne1_concat, ne2_concat, axes=1))+K.constant(0.0))

        else: # consider only the entity pools for "push-pull"
            print('entity only push-pull...')
            log_prob_eq = K.log(K.sigmoid(K.batch_dot(eq1_embs, eq2_embs, axes=1))+K.constant(0.0))
            log_prob_ne = K.log(K.sigmoid(-K.batch_dot(ne1_embs, ne2_embs, axes=1))+K.constant(0.0))
            sigmoid_ne = -K.batch_dot(ne1_embs, ne2_embs, axes=1)
        ...
        sg_train = -K.mean(log_prob_positive + log_prob_negative)
        eq_train = -K.mean(log_prob_eq)
        ne_train = -K.mean(log_prob_ne)
        sigs_train = sigmoid_ne
        ...
        if usePatPool:
            self.keras_train = K.function(
                [mb_in_indices, mb_out_indices, mb_negsamples_indices,
                 ...
                 pool_eq_ep_ent, pool_eq_ep_pat,
                 pool_ne_ep_ent, pool_ne_ep_pat],
                [loss_train,sg_train,eq_train,ne_train,sigs_train],
