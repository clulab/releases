...
from w2vboot_classifier_load_vectors import Word2vec
...
class Emboot:
    ...
    def __init__(self):
        ...
        ### Output files
        #########################################################################
        parser.add_argument('--logfile', dest='logfile', default='pools_output_interpretable_load_vectors.txt', help='entities promoted per epoch')
        ...
        print("CONTEXT IDS ", self.context_ids)
    ...
    def initialize_model(self, pools_entities, pools_contexts):
        ...
        self.model = Word2vec(self.entity_vocab, self.context_vocab,
                              ...
                              self.categories,
                              self.usePatPool,
                              self.initGigaEmbed, self.gigaW2vEmbed, self.lookupGiga, pools_entities, pools_contexts)
...
###################################################################################################
    ...
    def interpretable_prediction(self, entityId, patternIds): 
        ...
        if len(patternIds_in_pool) > 0:
            ...
            #self.debug_log.write("[interpret] " + "\te=" + self.entity_vocab.get_word(entityId)
                  #               +"\tsz=" + str(len(softmax_res)) + "\tsm=" + str(avg_softmax_res) + "\tno-" + str(noisy_or_score) + "\tp-" + str(prediction) + "\n")
    ...
    def build_interpretable_model(self): 
        ...
        for entityId, patternIds in self.entityToPatternsIdx.items():# for every entity in the dataset 
            ...
            prediction, noisy_or_score = self.interpretable_prediction(entityId, patternIds)
            if prediction != -1:
    ...
    def print_interpretable_model(self, entity_predictions, entity_scores, interpretable_model_log):
        ...
        for entId in entity_predictions.keys():
            ...
            interpretable_model_log.write("1" + '\t' + ent + "\t" + prediction + "\t" + str(entity_scores[entId]) + "\n")

    def get_pools_contexts(self, filename):
        pools_contexts_nat_lang = {}
        pools_contexts = {}
        categories = self.categories
        for cat in categories:
            pools_contexts[cat] = []
            pools_contexts_nat_lang[cat] = []

        with open(filename) as f:
            data = f.readlines()
            for line in data:
                #line = line.strip()
                if line.startswith("Epoch") or len(line.split('\t')) < 2:
                    continue
                else:
                    splitOnTab = line.split("\t")
                    categ = line.split("\t")[0]
                    pattern_list = line.split("\t")[1:]
                    new_pattern_list = [self.context_vocab.word_to_id[pattern.strip()] for pattern in pattern_list]
                  
                    pools_contexts[categ].extend(new_pattern_list)
                    pools_contexts_nat_lang[categ].extend(pattern_list)

        return pools_contexts


    def get_pools_entities(self, filename):
        pools_contexts_nat_lang = {}
        pools_contexts = {}
        categories = self.categories
        for cat in categories:
        ...
        with open(filename) as f:
            ...
            for line in data:
                ...
                if line.startswith("Epoch") or len(line.split('\t')) < 2:
                    ...
                    categ = line.split("\t")[0]
                    pattern_list = line.split("\t")[1:]
                    new_pattern_list = [self.entity_vocab.word_to_id[pattern.strip()] for pattern in pattern_list]
...
if __name__ == '__main__':
    ...
    emboot = Emboot()
    pools_contexts = emboot.get_pools_contexts("edited_patterns.txt")
    pools_entities = emboot.get_pools_entities("pools_output_interpretable.txt")

    emboot = Emboot()
    emboot.interpretable_model_log = open(emboot.args.logfile+"_interpretable_model.txt", 'w')

    emboot.initialize_model(pools_entities,pools_contexts)

    print("Building the interpretable model")
            ## Build the interpretable model
    entity_predictions, entity_scores = emboot.build_interpretable_model()

            ## Score the interpretable model
    emboot.print_interpretable_model(entity_predictions, entity_scores, emboot.interpretable_model_log)
