...
/**
 * Discourse ranking model using a Cusp-based representation scheme for locating discourse features
 * User: peter
 * Date: 5/1/13
 */

class DiscourseModelCusp(props:Properties) extends RankingModel {
  val termFilter = new TermFilter()
  val indexDir = props.getProperty("index")
  val qProcessor = new QuestionProcessor(props)
  val enableWord2Vec = StringUtils.getBool(props, "ranker.enable_word2vec_feature", false)
  var modelWord2Vec:Word2VecModel = null
  if (enableWord2Vec == true) modelWord2Vec = new Word2VecModel(props)
  ...
  def mkFeaturesCuspDispatch (answer:AnswerCandidate,
    ...
    val featuresMarkers = mkFeaturesCusp(answer, question)
    var combined = featuresMarkers
    if (enableWord2Vec == true) {
      val featuresWord2Vec = modelWord2Vec.mkFeatures(answer, question, None)._1
      combined += featuresWord2Vec
    }
    return combined
  }
