...
class BaseEncodeAggregateModel(pl.LightningModule):
    ...
    import collections
    BatchType = collections.namedtuple('BatchType', ['sentence', 'start_highlight', 'end_highlight', 'current_rule', 'next_potential_rule', 'is_it_correct']) 
    EncodeAggregateBatch = List[BatchType]
    
    def __init__(self, encoder: BaseEncoder, aggregator: BaseAggregator, hparams = {}):
        super().__init__()
        self.hparams = hparams
    ...
    """
    @staticmethod
    def collate_fn(tokenizer, symbols, symbol_tensors, parser: QueryParser, batch: List[AggregatorBatch], collate_fn):
        # Flatten everything
...
"""
    TODO specify in hparams place to load or create a new encoder/aggregator
    encoder -> something deriving from BaseModel, but without the projection head
"""
class OldBaseAggregator(pl.LightningModule):
    def __init__(self, encoder, aggregator, hparams={}):
        super().__init__()
        self.hparams = hparams
        # Encode each tuple (for example (sentence_1, current_rule, potential_next_rule_1))
        self.encoder = encoder

        # Since each continuation has to be overally good for all the sentences (sentence_1, .., sentence_k)
        # we use an aggregator to encode the encoded states for a continuation in those k rules
        self.aggregator = aggregator
    ...
    def forward(self, batch, return_logits=False):
        ...
        if 'number_of_sentences' not in batch:
            raise ValueError(f'The current training regime is {self.training_regime}, but the batch does not contain the required keys')
        
        number_of_sentences   = batch['number_of_sentences']

        # Split using the number of sentences per pattern
        # Will be a list of tensors, where each tensor has (<number_of_sentences>, hidden_size)
        # The <number_of_sentences> can be variable between the list's elements. The hidden size
        # is the same
        encoded_split = list(encoded.split([y for x in number_of_sentences for y in x]))
        
        # Calculating the len for each transition (number of sentences) and max length
        lens = [y for x in number_of_sentences for y in x]

        # Map from (batch_size_like, max_len, hidden_size) to (batch_size_like, hidden_size)
        # batch_size_like is not exactly batch_size because different transition steps
        # can have a different number of potential rules. So a batch size of 2, where
        # each transition has 5, respectively 7 potential next_rules will have a 
        # batch_size_like equal to 12
        aggregated = self.aggregator(encoded_split, lens, self.device)
        projected  = self.projection(self.dropout(aggregated))
    ...
    def training_step(self, batch, batch_idx):
        # First is correct
        logits = self.forward(batch, return_logits=True).squeeze(dim=1)
        number_of_transitions  = batch['number_of_transitions']
        number_of_sentences    = batch['number_of_sentences']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        scores_per_step = list(scores.split(sizes_in_logit))
        mrr = calc_reciprocal_ranks(scores_per_step)
        return {'val_loss': loss, 'pred': pred, 'gold': gold, 'partial_rr': mrr}
    ...
    def configure_optimizers(self):
        named_params = list(self.named_parameters())
        # named_params = [*list(self.aggregator.named_parameters(), *list(self.projection.named_parameters()))]
        params = [
            {
                'params': [p for n,p in named_params if not any(nd in n for nd in config.NO_DECAY)],
        ...
        if 'use_scheduler' in self.hparams and self.hparams['use_scheduler']:                
            return (
                ...
                [{
                    # 'scheduler': get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=self.hparams['num_training_steps']//16, num_training_steps=self.hparams['num_training_steps'],),
                    # 'scheduler': get_linear_schedule_with_warmup(optimizer, num_warmup_steps=self.hparams['num_training_steps']//2, num_training_steps=self.hparams['num_training_steps'],),
                    'scheduler': torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=config.LEARNING_RATE/5, max_lr = config.LEARNING_RATE, mode='triangular2', cycle_momentum=False, step_size_up=self.num_training_steps//2),
                    'interval': 'step',
                    'frequency': 1,
                    'strict': True,
                    'reduce_on_plateau': False, 
    ...
    def collate_fn_rolled(tokenizer, symbols, symbol_tensors, parser: QueryParser, batch: List[List[List[Dict]]], collate_fn):
        return BaseModel.collate_fn_rolled(tokenizer, symbols, symbol_tensors, parser, batch, collate_fn)
...
class AverageEncoder(nn.Module):
    ...
    """
    :param batch -> list of 2d tensors. The first dimension is variable
                    (number_of_sentences, hidden_size)
    :param lens  -> lengths
    :returns (batch_size_like, output_size)

    Apply a mean operation over each 2d tensor from batch. The mean is applied
    over the first dimension, transforming a (number_of_sentence, hidden_size) to 
    (hidden_size)
    """
    def forward(self, batch, lens, device):
        encoded = torch.cat([x.mean(dim=0).unsqueeze(0) for x in batch], dim=0)
        return encoded
...
class MaxEncoder(nn.Module):
    ...
    Apply a max operation over each 2d tensor from batch. The max is applied
    over the first dimension, transforming a (number_of_sentence, hidden_size) to 
    (hidden_size)
    """
    def forward(self, batch, lens, device):
        encoded = torch.cat([x.max(dim=0).values.unsqueeze(0) for x in batch], dim=0)
        return encoded
...
class LSTMEncoder(nn.Module):
    ...
    :param batch -> list of 2d tensors. The first dimension is variable
    :param lens  -> lengths
    :returns (batch_size_like, max_len, output_size)
             If bidirectional, the output_size is 2 * hidden_size
    """
    def forward(self, batch, lens, device):
        max_length = max(lens)
        ...
        encoded_sequence = []
        for enc in batch:
            if enc.shape[0] != max_length:
                encoded_sequence.append(torch.cat([enc, torch.zeros(max_length - enc.shape[0], enc.shape[-1]).to(device)], dim=0).unsqueeze(0))
            else:
                encoded_sequence.append(enc.unsqueeze(0))
        encoded_sequence = torch.cat(encoded_sequence, dim=0)

        packed_encoded_sequence = torch.nn.utils.rnn.pack_padded_sequence(encoded_sequence, batch_first=True, lengths=lens, enforce_sorted=False)
        _, (last_hidden, _) = self.lstm(packed_encoded_sequence)
