...
class ExperimentOpenbook():
    ...
    def run_all_exp(self):
        ...
        final_result_dict = {
            ...
            "reg":lrClassifier.reg
        }
        return np.mean(hybrid_lr_mrr), final_result_dict
...
class ExperimentSquad():
    ...
    def run_all_splits(self):
        ...
        return np.mean(np.concatenate(hybrid_lr)), all_results
...
def generate_trained_regs():
    feature_pos = np.arange(7)

    experimentOpenbook = ExperimentOpenbook(feature_pos)
    ob_mrr, ob_result = experimentOpenbook.run_all_exp()

    experimentSquad = ExperimentSquad(feature_pos)
    squad_mrr, squad_result = experimentSquad.run_all_splits()

    experimentNQ = ExperimentNQ(feature_pos)
    nq_mrr, nq_result = experimentNQ.run_all_splits()

    with open(generated_data_path+"hybrid_feature_analysis/results.pcikle", "wb") as handle:
        pickle.dump({"openbook":ob_result,"squad":squad_result,"nq":nq_result}, handle)


def generate_feature_histogram():

    with open(generated_data_path+"hybrid_feature_analysis/results.pcikle", "rb") as handle:
        all_result_dict = pickle.load(handle)

    def get_squad_nq_statistics(results):
        intercept_list = []
        coef_list = []
        mrrs = []
        for result_dict in results:
            ...
            coef_list.append(reg.coef_.flatten())

            mrrs.append(np.mean(result_dict["hybrid_lr"]["mrr"]))
        ...
        all_num_array = np.concatenate([coef_list, intercept_list], axis = 1)

        #print("all num size:", all_num_array.shape)

        return np.mean(all_num_array, axis =0), np.std(all_num_array, axis = 0), sum(mrrs)/len(mrrs)

    def get_openbook_nums(openbook_result_dict):
        reg = openbook_result_dict["reg"]

        intercept = reg.intercept_
        coef = reg.coef_.flatten()

        return np.concatenate([coef, intercept]),  np.mean(openbook_result_dict["hybrid_lr"]["mrr"])

    ob_coef, ob_mrr = get_openbook_nums(all_result_dict["openbook"])
    squad_coef_mean, squad_coef_std, squad_mrr = get_squad_nq_statistics(all_result_dict["squad"])
    nq_coef_mean, nq_coef_std , nq_mrr= get_squad_nq_statistics(all_result_dict["nq"])

    plt.figure()
    x_range = np.arange(8)
    width = 0.2

    plt.bar(x_range-width*1, ob_coef, width, label="OpenBookQA")
    plt.bar(x_range, squad_coef_mean, width, yerr = squad_coef_std, label = "ReQA SQuAD")
    plt.bar(x_range+width*1, nq_coef_mean, width, yerr = nq_coef_std, label = "ReQA NQ")

    plt.ylabel("coefficient magnitude")
    plt.xticks(x_range, ["coef1\n(top1)", "coef2\n(top2)", "coef3\n(top4)", "coef4\n(top8)", "coef5\n(top16)","coef6\n(top32)","coef7\n(top64)","intercept"])

    plt.legend()

    print("mrrs:", ob_mrr, squad_mrr, nq_mrr)
