...
class EvalBase():
    ...
    def _fill_results_dict_ann(self, gold_label, query_embd, sim, result_dict):
        self.t0 = time.time()
        nearest_neighbor = list(sim.nearest(query_embd, self.n_top_doc_save))
        self.t1 = time.time()

        gold_fact_ranking = nearest_neighbor.index(gold_label) if gold_label in nearest_neighbor else -1
        sample_mrr = 1 / (gold_fact_ranking + 1) if gold_label in nearest_neighbor else 0

        result_dict["gold_fact_index"].append(gold_label)
        result_dict["gold_fact_ranking"].append(gold_fact_ranking)  # get the gold fact ranking of each query
        result_dict["gold_fact_score"].append(0)
        result_dict["mrr"].append(sample_mrr)

        result_dict["top_facts"].append(nearest_neighbor)
        result_dict["top_scores"].append([0])

        result_dict["search_time"].append(self.t1 - self.t0)
        result_dict["sort_time"].append(0)

        return 0

    def _eval_ann(self, query_embds, fact_embds, instances_list):
        result_dict = {"gold_fact_index":[], "gold_fact_ranking":[], "gold_fact_score":[], "mrr":[], "top_facts":[], "top_scores":[], "search_time":[], "sort_time":[]}

        query_embds = query_embds.tolist()
        fact_embds = fact_embds.tolist()
        fact_embds = [(idx, fact_embd) for idx, fact_embd in enumerate(fact_embds)]

        sim = SimpleNeighbors(self.embd_dim_size)  # This 3 is the dimension of data.
        ...
        sim.build(n=20)  # here we can control the size of the tree, it controls the speed and accuracy of search.
        ...
        search_time = []
        mrr = []
        for query_idx, query_embd in enumerate(query_embds):

            self._fill_results_dict_ann(instances_list[query_idx][self.answer_field], query_embd, sim, result_dict)
...
class EvalOpenbook(EvalBase):
    ...
    def eval_dev(self):
        ...
        dev_result_dict = self._eval_ann(query_embds_dev, self.fact_embds, self.instances_list["dev_list"])

        dev_result_dict = self._trim_result_dict(dev_result_dict, reduce_precision = True)
        with open("openbook_dev_useqa_ann_result_dict.pickle", "wb") as handle:
    ...
    def eval_test(self):
        ...
        test_result_dict = self._eval_ann(query_embds_test, self.fact_embds, self.instances_list["test_list"])

        test_result_dict = self._trim_result_dict(test_result_dict, reduce_precision=True)
        with open("openbook_test_useqa_ann_result_dict.pickle", "wb") as handle:
...
class EvalSQuAD(EvalBase):
    ...
    def eval_dev(self):
        ...
        dev_result_dict = self._eval_ann(query_embds_dev, self.fact_embds, dev_list)

        dev_result_dict["dev_index_in_train_list"] = dev_query_idx_in_train_list

        dev_result_dict = self._trim_result_dict(dev_result_dict, reduce_precision=True)
        with open("squad_dev_useqa_ann_result_dict.pickle", "wb") as handle:
            pickle.dump(dev_result_dict, handle)
    ...
    def eval_test(self):
        ...
        test_result_dict = self._eval_ann(query_embds_test, self.fact_embds, self.instances_list["dev_list"])

        test_result_dict = self._trim_result_dict(test_result_dict, reduce_precision=True)
        with open("squad_test_useqa_ann_result_dict.pickle", "wb") as handle:
            pickle.dump(test_result_dict, handle)
...
class EvalNQ(EvalBase):
    ...
    def eval_test(self):
        ...
        test_result_dict = self._eval_ann(query_embds_test, self.fact_embds, self.instances_list["train_list"])

        test_result_dict = self._trim_result_dict(test_result_dict, reduce_precision=True)
        with open("nq_test_useqa_ann_result_dict.pickle", "wb") as handle:
            pickle.dump(test_result_dict, handle)
...
def check_result_dict_pickle(dataset, partition, debug=True):
    ...
    with open(dataset+"_"+partition+"_useqa_ann_result_dict.pickle" ,"rb") as handle:
...
def main(dataset):
    ...
    if dataset=="squad":
        ...
        check_result_dict_pickle(dataset, "dev")
        check_result_dict_pickle(dataset, "test", False)
